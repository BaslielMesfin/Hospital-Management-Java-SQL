...
class Patient
{
    ...
    public void addNewPatient()
    {
        ...
        try (PreparedStatement preparedStatement = DatabaseConnection.connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS))
        {
            ...
            if (rowaffected>0)
            {
                ...
                {
                    patientid = generatedKeys.getInt("patientid");
                }
                else
                {
...
class Appointment
{
    ...
    {
        if (choice2==1) // new patient
        {
            patient.addNewPatient();
        }
        else if (choice2==2) // existing patient
        {
    ...
    public void existingAppointment()
    {
        ...
        try (PreparedStatement preparedStatement = DatabaseConnection.connection.prepareStatement(get_id_sql))
        {
            preparedStatement.setString(1, fname);
            preparedStatement.setString(2, lname);
            ResultSet resultSet = preparedStatement.executeQuery();
            ...
            if (resultSet.next())
            {
                ...
                try(PreparedStatement appointmentStatement = DatabaseConnection.connection.prepareStatement(appt_sql))
                {
